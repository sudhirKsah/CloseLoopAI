...
function Dialog({
  ...
  return (
    <div
      ...
      aria-modal="true"
    >
      <div className="relative w-full max-w-lg rounded-2xl border border-white/10 bg-[#15151a] p-6 shadow-2xl">
        <button
...
const field =
  "mt-1.5 h-10 w-full rounded-xl border border-white/10 bg-white/[.04] px-3 text-sm text-white outline-none focus:border-emerald-300/60";
    ...
    <>
      {open && (
        <Dialog close={() => setOpen(false)}>
          <p className="text-lg font-semibold">Add a meeting</p>
          <p className="mt-1 text-sm text-zinc-500">
            CloseLoop will send its Recall bot to the meeting link.
          </p>
...
export function TaskCreateDialog({
  ...
  return (
    <>
      {open && (
        <Dialog close={() => setOpen(false)}>
          <p className="text-lg font-semibold">Create internal task</p>
          <p className="mt-1 text-sm text-zinc-500">
            Use this for work not created from a meeting extraction.
          </p>
          <form className="mt-6 space-y-4" onSubmit={submit}>
            <label className="block text-xs text-zinc-400">
              Task title
