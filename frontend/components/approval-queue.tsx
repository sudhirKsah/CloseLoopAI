...
export function ApprovalQueue() {
  ...
  const review = async (
    ...
    try {
      await api(`/task-candidates/${candidate.id}/review`, {
        method: "POST",
  ...
  return (
    <main className="p-5 lg:p-8">
      ...
      <h1 className="mt-2 text-3xl font-semibold">Task approvals</h1>
      <p className="mt-2 text-sm text-zinc-500">
        Only lower-confidence extracted commitments require a decision.
      </p>
                ...
                <div className="flex flex-wrap items-start justify-between gap-4">
                  <div>
                    ...
                    <p className="mt-3 text-xs text-zinc-500">
                      Owner: {candidate.owner ?? "Unassigned"} · Deadline:{" "}
                      {candidate.deadline
                        ? new Date(candidate.deadline).toLocaleDateString()
                        : "Not specified"}
                    </p>
                  </div>
                  <p className="rounded-full bg-amber-300/10 px-3 py-1 text-xs text-amber-200">
                    {Math.round(candidate.confidence * 100)}% confidence
                  </p>
                </div>
                <pre className="mt-4 whitespace-pre-wrap rounded-xl bg-black/20 p-3 text-xs text-zinc-500">
