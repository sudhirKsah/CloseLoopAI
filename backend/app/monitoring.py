...
class TaskConnector:
    ...
    async def collect(
        self, session: AsyncSession, task: Task, since: datetime
    ) -> list[Signal]:
...
class GithubConnector(TaskConnector):
    ...
    async def collect(self, session, task, since):
            ...
            (
                await session.execute(
                    select(TaskActivityMatch).where(
                        ...
                        TaskActivityMatch.created_at >= since,
                    )
                )
            )
            .scalars()
        ...
        return [
            Signal(self.name, SignalKind.PROGRESS, m.created_at, m.reason, m.confidence)
            for m in matches
        ]
...
class IssueTrackerConnector(TaskConnector):
    ...
    async def collect(self, session, task, since):
            ...
            (
                await session.execute(
                    select(ExternalTaskMapping).where(
                        ExternalTaskMapping.task_id == task.id
                    )
                )
            )
            .scalars()
        ...
        for mapping in mappings:
            ...
            if any(word in state for word in ("blocked", "impediment", "on hold")):
                out.append(
                    Signal(
                        "jira_or_linear",
                        SignalKind.BLOCKED,
                        mapping.last_synced_at or since,
                        f"External issue status: {mapping.last_status}",
                        0.9,
                    )
                )
            elif any(word in state for word in ("done", "closed", "completed")):
                out.append(
                    Signal(
                        "jira_or_linear",
            ...
            elif mapping.last_synced_at and mapping.last_synced_at >= since:
                out.append(
                    Signal(
                        "jira_or_linear",
                        SignalKind.PROGRESS,
                        mapping.last_synced_at,
                        "Issue updated during the monitoring window",
                        0.7,
                    )
                )
        return out
...
class CalendarConnector(TaskConnector):
    ...
    async def collect(self, session, task, since):
            ...
            (
                await session.execute(
                    select(CalendarEvent).where(
                        CalendarEvent.owner_id == task.owner_id,
                        CalendarEvent.starts_at
                        <= datetime.now(UTC) + timedelta(days=2),
                        CalendarEvent.ends_at >= datetime.now(UTC),
                    )
                )
            )
            .scalars()
        ...
        hours = sum(
            (event.ends_at - event.starts_at).total_seconds() / 3600 for event in events
        )
        if hours >= 7:
            return [
                Signal(
                    self.name,
                    SignalKind.CALENDAR_CONSTRAINT,
                    datetime.now(UTC),
                    f"Owner has {hours:.1f} scheduled calendar hours in the next two days",
                    0.8,
                )
            ]
        return []
...
class HistoryConnector(TaskConnector):
    ...
    async def collect(self, session, task, since):
        ...
        if history and history.to_state == TaskState.BLOCKED:
            return [
                Signal(
                    self.name,
                    SignalKind.BLOCKED,
                    history.created_at,
                    history.reason or "Task marked blocked in status history",
                    0.85,
                )
            ]
        return []
...
class MonitoringPolicy:
    def assess(self, task: Task, signals: list[Signal], now: datetime) -> Assessment:
        blockers = [
            signal.evidence for signal in signals if signal.kind == SignalKind.BLOCKED
        ]
        if any(signal.kind == SignalKind.COMPLETED for signal in signals):
            return Assessment(
                TaskState.COMPLETED,
                18,
                None,
                "Completion verified by a connected system.",
                blockers,
            )
        if blockers:
            return Assessment(
        ...
        if any(signal.kind == SignalKind.PROGRESS for signal in signals):
            return Assessment(
        ...
        if inactive:
            return Assessment(
                ...
                -6,
                "nudge",
                "No progress signal has been recorded for three days.",
                blockers,
            )
...
async def monitor_task(
    ...
    for connector in CONNECTORS:
        try:
            signals.extend(
                await connector.collect(session, task, now - timedelta(days=1))
            )
        except Exception:
            continue  # individual integrations never block other signals
    assessment = MonitoringPolicy().assess(task, signals, now)
    before = task.state
    task.state = assessment.state
    task.execution_score = max(
        0, min(100, task.execution_score + assessment.score_delta)
    )
