...
def tone_for(task: Task, prior: int) -> Tone:
    if prior >= 5:
        return Tone.FOUNDER_ESCALATION
    if prior >= 4:
        return Tone.MANAGER_ESCALATION
    if prior >= 3:
        return Tone.ESCALATION_WARNING
    if prior >= 2:
        return Tone.FIRM
    if prior >= 1:
...
async def create_reminder(
    ...
    if any(item.body_hash == body_hash for item in prior):
        return None
    reminder = Reminder(
        task_id=task.id,
...
def _compose(
    ...
    prefix = {
        Tone.FRIENDLY: "Quick check-in",
        Tone.REMINDER: "Reminder",
        Tone.FIRM: "Action needed",
        Tone.ESCALATION_WARNING: "Escalation warning",
        Tone.MANAGER_ESCALATION: "Manager attention requested",
        Tone.FOUNDER_ESCALATION: "Founder escalation",
    }[tone]
