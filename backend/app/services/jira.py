...
class JiraClient:
    ...
    async def _token(self, body: dict) -> dict:
        async with httpx.AsyncClient(timeout=20) as client:
            response = await client.post(
                "https://auth.atlassian.com/oauth/token", json=body
            )
    ...
    async def request(
        self, token: str, cloud_id: str, method: str, path: str, **kwargs: object
    ) -> dict:
        async with httpx.AsyncClient(timeout=20) as client:
            response = await client.request(
                method,
                f"https://api.atlassian.com/ex/jira/{cloud_id}/rest/api/3/{path.lstrip('/')}",
                headers={
    ...
    async def projects(self, token: str, cloud_id: str) -> list[dict]:
        return await self.request(token, cloud_id, "GET", "project/search")
    ...
    async def update_issue(
        self, token: str, cloud_id: str, issue_id: str, fields: dict
    ) -> dict:
        return await self.request(
            token, cloud_id, "PUT", f"issue/{issue_id}", json={"fields": fields}
        )

    async def issue(self, token: str, cloud_id: str, issue_id: str) -> dict:
        return await self.request(
