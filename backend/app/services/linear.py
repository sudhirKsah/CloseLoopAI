...
class LinearClient:
    ...
    async def token_for(self, session: AsyncSession, integration: Integration) -> str:
        ...
        if expires_at and expires_at <= datetime.now(UTC) + timedelta(minutes=2):
            ...
            credential.refresh_token_encrypted = vault.encrypt(
                refreshed.get(
                    "refresh_token",
                    vault.decrypt(credential.refresh_token_encrypted or ""),
                )
            )
    ...
    async def graphql(
        self, token: str, query: str, variables: dict | None = None
    ) -> dict:
    ...
    async def projects(self, token: str) -> list[dict]:
        return (await self.graphql(token, "{ projects { nodes { id name } } }"))[
            "projects"
        ]["nodes"]
    ...
    async def update_issue(self, token: str, issue_id: str, input_: dict) -> dict:
        return (
            await self.graphql(
                token,
                "mutation($id:String!,$input:IssueUpdateInput!){ issueUpdate(id:$id,input:$input){ issue { id identifier status { name } } } }",
                {"id": issue_id, "input": input_},
            )
        )["issueUpdate"]["issue"]

    async def issue(self, token: str, issue_id: str) -> dict:
        return (
            await self.graphql(
                token,
                "query($id:String!){ issue(id:$id){ id identifier title updatedAt status { name type } } }",
                {"id": issue_id},
            )
        )["issue"]
