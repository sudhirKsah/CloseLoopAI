...
class WeeklyReportService:
    async def generate(
        ...
        start = period_end - timedelta(days=7)
        ...
        if not report:
            report = WeeklyReport(
                workspace_id=workspace_id, period_start=start, data=metrics
            )
            session.add(report)
            await session.flush()
    ...
    def _html(
        self, metrics: dict, insights: list[dict], start: datetime, end: datetime
    ) -> str:
        rows = "".join(
            f"<li><b>{item['key'].replace('_',' ').title()}</b>: {item['value']} <small>confidence {item['confidence']:.0%}</small><br>{item['explanation']}</li>"
            for item in insights
        )
        chart = "".join(
            f"<div style='height:{max(8,min(160,value*6))}px'></div>"
            for value in [
                metrics["organization_summary"]["tasks"],
                metrics["organization_summary"]["completed"],
