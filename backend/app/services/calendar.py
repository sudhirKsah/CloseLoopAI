...
class CalendarClient:
    ...
    def scopes(self) -> str:
        return (
            ...
            if self.provider == IntegrationProvider.GOOGLE_CALENDAR
            else "offline_access Calendars.Read"
        )
    ...
    async def token_for(self, session: AsyncSession, integration: Integration) -> str:
        ...
        if credential.expires_at and credential.expires_at <= datetime.now(
            ...
            async with httpx.AsyncClient() as c:
                response = await c.post(url, data=data)
                response.raise_for_status()
                refreshed = response.json()
            credential.access_token_encrypted = vault.encrypt(refreshed["access_token"])
            credential.refresh_token_encrypted = vault.encrypt(
                refreshed.get("refresh_token", refresh)
            )
...
def calendar_health(events: list[dict], now: datetime | None = None) -> dict:
    ...
    for event in events:
        ...
        if (
            event.get("showAs") == "oof"
            or "vacation" in event.get("summary", event.get("subject", "")).casefold()
            or "out of office"
            in event.get("summary", event.get("subject", "")).casefold()
        ):
            oof = True
        if start and end:
            try:
    ...
    return {
        ...
        "out_of_office": oof,
        "vacation": oof,
        "meeting_overload": busy >= 6 or meetings >= 6,
        "recommended_due_days": 5 if oof or busy >= 6 else 3,
    }


async def sync_calendar(
    session: AsyncSession, integration: Integration, user_id: str
) -> dict:
    client = CalendarClient(integration.provider)
    events = await client.events(
        await client.token_for(session, integration),
        datetime.now(UTC),
    ...
    for event in events:
        ...
        session.add(
            CalendarEvent(
                ...
                title=event.get("summary") or event.get("subject") or "Busy",
