...
class SlackClient:
    ...
    async def team_users(self, token: str) -> list[dict]:
        async with httpx.AsyncClient() as client:
            ...
            data = response.json()
        if not data.get("ok"):
            raise RuntimeError(data.get("error", "Slack users.list failed"))
        return data["members"]
    ...
    return [
        {
            ...
            "text": {
                "type": "mrkdwn",
                "text": f"*Task approval needed* · confidence {candidate.confidence:.0%}\n*Task:* {candidate.title}\n*Owner:* {owner}\n*Deadline:* {deadline}",
            },
        },
        {
            "type": "actions",
            "elements": [
                {
                    ...
                    "style": "primary",
                    "action_id": "closeloop_task_approve",
                    "value": str(candidate.id),
                },
                {
                    "type": "button",
                    "text": {"type": "plain_text", "text": "Edit"},
                    "action_id": "closeloop_task_edit",
                    "value": str(candidate.id),
                },
                {
                    "type": "button",
                    ...
                    "style": "danger",
                    "action_id": "closeloop_task_reject",
                    "value": str(candidate.id),
                },
            ],
        },
...
async def post_approval(token: str, channel: str, candidate: TaskCandidate) -> dict:
    async with httpx.AsyncClient() as client:
        response = await client.post(
            ...
            json={
                "channel": channel,
                "blocks": approval_blocks(candidate),
                "text": f"Task approval needed: {candidate.title}",
            },
        )
        response.raise_for_status()
