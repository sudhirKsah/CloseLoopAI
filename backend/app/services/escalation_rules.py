...
class RuleEngine:
    async def facts(self, session: AsyncSession, task: Task, now: datetime) -> dict:
        prior = (
            await session.execute(
                select(func.count(Reminder.id)).where(Reminder.task_id == task.id)
            )
        ).scalar_one()
        overdue_days = (
            max(0, (now - task.due_at).days) if task.due_at and task.due_at < now else 0
        )
    ...
    async def evaluate(
        self, session: AsyncSession, task: Task, now: datetime | None = None
    ) -> list[Escalation]:
        ...
        rules = (
            (
                await session.execute(
                    ...
                    .where(
                        EscalationRule.workspace_id == task.workspace_id,
        ...
        facts = await self.facts(session, task, now)
        created = []
        for rule in rules:
            if not self.matches(rule.conditions, facts):
                continue
            action = rule.action
            ...
            if action.get("type") == "escalate":
                ...
                if (
                    ...
                    and not (
                        await session.execute(
                            select(Escalation.id).where(
                                ...
                                Escalation.level == action.get("level", 1),
                    ...
                    escalation = Escalation(
                        task_id=task.id,
                        escalated_to_id=recipient,
                        level=action.get("level", 1),
                        reason=f"{rule.name}: {facts}",
                    )
    ...
    async def _recipient(self, session: AsyncSession, task: Task, action: dict):
        ...
        if action.get("target") == "founder":
            workspace = await session.get(Workspace, task.workspace_id)
            return workspace.settings.get("founder_user_id") if workspace else None
        return task.owner_id


DEFAULT_RULES = [
    {
        "name": "No progress 3 days",
        "priority": 10,
        "conditions": {"inactive_days": {"gte": 3}},
        "action": {"type": "reminder", "channel": "slack"},
    },
    {
        "name": "No progress 7 days",
        "priority": 20,
        "conditions": {"inactive_days": {"gte": 7}},
        "action": {"type": "escalate", "target": "manager", "level": 2},
    },
    {
        "name": "No progress 14 days",
        "priority": 30,
        "conditions": {"inactive_days": {"gte": 14}},
        "action": {"type": "escalate", "target": "founder", "level": 3},
    },
    {
        "name": "Repeated missed deadlines",
        "priority": 40,
        "conditions": {"missed_deadlines": {"gte": 3}},
        "action": {"type": "escalate", "target": "manager", "level": 2},
    },
]
