...
class GithubClient:
    ...
    async def repositories(self, token: str) -> list[dict]:
        return await self.get(token, "/user/repos", {"per_page": 100, "affiliation": "owner,collaborator,organization_member", "sort": "updated"})  # type: ignore[return-value]
    ...
    async def activity(
        ...
        commits = await self.get(token, f"/repos/{full_name}/commits", params)
        prs = await self.get(
            token,
            f"/repos/{full_name}/pulls",
            {"state": "all", "sort": "updated", "direction": "desc", "per_page": 100},
        )
        issues = await self.get(
            token,
            f"/repos/{full_name}/issues",
            {"state": "closed", "since": since.isoformat(), "per_page": 100},
        )
        return (
            ...
            + [("issue_closed", x) for x in issues]
        )
...
async def sync_repo_activity(session: AsyncSession, repo: GithubRepo) -> int:
    ...
    for kind, payload in await client.activity(token, repo.full_name, since):
        ...
        if exists:
            continue
        author = (payload.get("author") or payload.get("user") or {}).get("login")
...
async def map_activity(
    ...
    text = " ".join(
        str(x)
        for x in [
            activity.payload.get("commit", {}).get("message", ""),
            activity.payload.get("title", ""),
            activity.payload.get("body", ""),
        ]
    ).casefold()
    tasks = (
        (
            await session.execute(
                select(Task).where(
                    Task.workspace_id
                    == (
                        await session.get(Integration, repo.integration_id)
                    ).workspace_id,
        ...
        if overlap:
            ...
            session.add(
                TaskActivityMatch(
                    ...
                    reason="Task-title keyword overlap in selected repository",
                )
            )
