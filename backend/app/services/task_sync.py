...
async def sync_task(
    session: AsyncSession, task: Task, integration: Integration
) -> ExternalTaskMapping:
    mapping = (
        await session.execute(
            select(ExternalTaskMapping).where(
                ExternalTaskMapping.task_id == task.id,
                ExternalTaskMapping.integration_id == integration.id,
            )
        )
    ).scalar_one_or_none()
    if integration.provider == IntegrationProvider.JIRA:
        ...
        if mapping:
            await client.update_issue(
                ...
                integration.config["cloud_id"],
                mapping.external_id,
                {"summary": task.title, "description": task.description or ""},
            )
        ...
        external_id, external_key, status_ = (
            issue["id"],
            issue.get("key"),
    ...
    mapping.last_status, mapping.last_synced_at = status_, datetime.now(UTC)
    session.add(mapping)
...
async def sync_workspace(session: AsyncSession, workspace_id: str) -> int:
            ...
            await session.execute(
                select(Integration).where(
                    ...
                    Integration.provider.in_(
                        [IntegrationProvider.JIRA, IntegrationProvider.LINEAR]
                    ),
                )
            )
        )
    ...
    for task in tasks:
        for integration in integrations:
            try:
                await sync_task(session, task, integration)
                count += 1
