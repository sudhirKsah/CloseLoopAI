...
async def candidates(
    ...
    _: User = Depends(require_workspace_admin),
    ...
    return [
        {
            ...
            "evidence": x.evidence,
            "state": x.state.value,
            "task_id": str(x.task_id) if x.task_id else None,
        }
        for x in rows
    ]
...
async def candidate(
    ...
    if not item:
        raise HTTPException(404, "Task candidate not found")
    member = (
        await session.execute(
...
async def review(
    ...
    if body.reviewer_id != user.id:
        raise HTTPException(
            403, "Reviewer identity does not match the authenticated user"
        )
    ...
    member = (
        await session.execute(
            select(WorkspaceMember).where(
                WorkspaceMember.workspace_id == candidate_item.workspace_id,
                WorkspaceMember.user_id == user.id,
            )
        )
    ).scalar_one_or_none()
    if not member or member.role.value not in ("owner", "admin"):
        raise HTTPException(403, "Workspace administrator access required")
    try:
        item = await TaskApprovalService().review(
            session, candidate_id, body.decision, body.reviewer_id, body.edit
        )
