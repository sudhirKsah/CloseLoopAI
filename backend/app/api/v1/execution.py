...
async def create_rule(
    ...
    rule = EscalationRule(workspace_id=workspace_id, **body.model_dump())
    session.add(rule)
...
async def generate_report(
    ...
    report = await WeeklyReportService().generate(session, str(workspace_id))
    return {"id": str(report.id), "pdf_url": report.pdf_url, "data": report.data}
