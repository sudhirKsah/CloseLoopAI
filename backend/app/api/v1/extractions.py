...
async def check_transcript_access(
    session: AsyncSession, transcript_id: uuid.UUID, user: User
) -> None:
    ...
    result = (
        await session.execute(
            select(MeetingExtraction).where(
                MeetingExtraction.transcript_id == transcript_id
            )
        )
    ).scalar_one_or_none()
    if not result:
        raise HTTPException(404, "No extraction exists for this transcript")
    return {
        "id": str(result.id),
