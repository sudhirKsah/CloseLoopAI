...
async def create_bot(
    ...
    if not member or member.role.value not in ("owner", "admin"):
        raise HTTPException(
            status_code=403, detail="Workspace administrator access required"
        )
    meeting = Meeting(
        workspace_id=workspace_id,
        provider=body.provider or provider_for_url(str(body.meeting_url)),
        join_url=str(body.meeting_url),
        title=body.title,
        scheduled_at=body.join_at,
        status=MeetingStatus.JOINING,
    )
    session.add(meeting)
    await session.flush()
    try:
        recall_bot = await RecallClient().create_bot(
            meeting_url=str(body.meeting_url),
            bot_name=body.bot_name,
            join_at=body.join_at,
            metadata={
                "closeloop_meeting_id": str(meeting.id),
...
async def receive_event(
    ...
    secret = (
        settings.recall_svix_webhook_secret
        or settings.recall_workspace_verification_secret
    )
    event_id = RecallSignatureVerifier(secret).verify(
        {k.lower(): v for k, v in request.headers.items()}, raw_body
    )
    try:
    ...
    if not event_type:
        raise HTTPException(400, "Recall event type missing")
    statement = (
        insert(WebhookEvent)
        .values(
            provider="recall", event_id=event_id, event_type=event_type, payload=payload
        )
